...
$motif = mysqli_real_escape_string($conn, $_POST['nom']);

if(empty($motif)){
	echo "champ vide";
	exit();
}else{
	$sql="SELECT nom FROM  departement where nom='$motif'";
	...
	if($resultcheck == 0){
		...
		if($resultcheck > 0){
			?>	
                ...
                <form class="box" action="17_1_modifier_proprietes_departement_sql.php" method="POST">
