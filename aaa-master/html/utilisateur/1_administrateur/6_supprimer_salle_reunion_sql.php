...
$sql1 = "SELECT intitule FROM sallereunion where intitule='$nom'";
$result1=mysqli_query($conn , $sql1);
$resultcheck1=mysqli_num_rows($result1);
if($resultcheck1 > 0){
	$sql2 = "SELECT intitule FROM reservation where intitule='$nom'";
	$result2=mysqli_query($conn , $sql2);
	$resultcheck2=mysqli_num_rows($result2);
	if($resultcheck2 > 0){	
			echo "la salle est déja résérvé, vous ne pouvez pas la supprimer.";
	}else{
		$sql1= "DELETE FROM sallereunion WHERE intitule = '$nom'";
		if($conn->query($sql1) === TRUE){
			echo "la salle de reunion  a été supprimé.";
		}else{
			echo "Erreur: " . $sql . "<br/>" . $conn->error;
		}	
	}
}else{
	echo "cette salle  n'éxiste pas.";
	exit();
}
?>
</body>
