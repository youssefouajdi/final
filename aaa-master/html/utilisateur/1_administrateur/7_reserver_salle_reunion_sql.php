...
if(empty($login)){
	...
	if($resultcheck == 0){
		...
		if($datedebut>$datefin){
			echo "la date debut doit être inférieur à la date fin";
			exit();
		}else{
			$sql3 = "SELECT * FROM reservation WHERE intitule = '$intutile' and heuredebut = '$datedebut' and heurefin = '$datefin' and motif='$motif' and login='$login'";
			$result3=mysqli_query($conn , $sql3);
			$resultcheck3=mysqli_num_rows($result3);
			if($resultcheck3 > 0){
				echo "cette réservation existe déja.<br>";
				$sql2= "DELETE FROM demandereunion WHERE intutile = '$intutile' and heuredebut = '$datedebut' and heurefin = '$datefin' and motif='$motif' and login='$login'";
					if($conn->query($sql2) === TRUE){
						echo " la demande a été retiré de la liste.";
					}else{
						echo "Erreur: " . $sql . "<br/>" . $conn->error;
					}
				exit();
			}else{
				$sql ="INSERT INTO reservation (intitule,heuredebut, heurefin, motif, login)
				VALUES ('$intutile','$datedebut','$datefin','$motif','$login')";
				if($conn->query($sql) === TRUE){
					echo "la reservation est faite avec succes.<br>";
					$sql2= "DELETE FROM demandereunion WHERE intutile = '$intutile' and heuredebut = '$datedebut' and heurefin = '$datefin' and motif='$motif' and login='$login'";
					if($conn->query($sql2) === TRUE){
						echo "la demande a été supprimé.";
					}else{
						echo "Erreur: " . $sql . "<br/>" . $conn->error;
					} 
				}else{
